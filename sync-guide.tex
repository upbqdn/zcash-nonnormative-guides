...
shielded output on the chain (\emph{Ironwood Guide}, \S``Transmission and
...
\item Field \texttt{header} ``should always be unset (empty)'' per the
  ...
  (\S\ref{sec:sync-compact-scan}). What a header-verifying client would
  gain from the ZIP-221 chain-history commitment is the \emph{FlyClient
  Guide}'s subject.
...
\item Field \texttt{protoVersion} never varies on the wire.
  ...
  field---its lone non-zero literal (\texttt{zaino-fetch},
  \texttt{src/chain/block.rs}, line~218) sits in a parse-error
  \texttt{Debug} string and never reaches serialization; v0.5.0 removed
...
mechanics the \emph{Ironwood Guide} constructs in \S``Proof of ownership of
...
(ZIP~221, via \texttt{hashBlockCommitments}); no deployed client consumes
them (\emph{FlyClient Guide}, \S``The deployment gap'').
...
replays the chain's prefix (\emph{Ironwood Guide}, \S``Proof of ownership
...
recomputation --- is the \emph{Ironwood Guide}'s material (\S``Transmission
...
the \emph{Ironwood Guide}'s material (\S``Proof of ownership of \emph{some}
...
shards --- is the \emph{Ironwood Guide}'s construction (\S``Proof of
...
\emph{Ironwood Guide}'s (\S``Proof of ownership of \emph{some} valid
...
decryption on the device (\emph{Ironwood Guide}, \S``Transmission and
detection of a note: encryption, trial decryption, and synchronisation'').
...
\texttt{src/scanning.rs}), by the mechanism of the \emph{Ironwood Guide},
\S``Transmission and detection of a note: encryption, trial decryption, and
