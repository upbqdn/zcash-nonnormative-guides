...
\thispagestyle{empty}
\begin{abstract}
This is the fourth and final volume of a four-part series --- \emph{Math Guide}, \emph{Crypto Guide}, \emph{Halo~2 Guide}, \emph{Orchard Guide}. Assuming the three companion volumes, it develops Zcash's Orchard shielded-payment protocol: notes and note commitments, the commitment tree and the procedure by which a wallet obtains a membership path, nullifiers, the key hierarchy, value commitments and the RedPallas signature, note encryption and trial decryption, the formal Action statement the SNARK proves, and the end-to-end security reductions that convert knowledge soundness into balance, nullifier uniqueness, and zero knowledge --- closing with the Zcash proof-system lineage.
\end{abstract}
\clearpage
\tableofcontents
\clearpage
...
(the general-purpose proof system). Halo~2 has no notion of money; this volume converts it into a
shielded-payment protocol by fixing one specific relation --- the \emph{Action statement} --- that
every transaction must prove.

We develop the protocol from the problem it solves rather than from the symbols it uses. After
...
membership path); how the protocol prevents double-spending (nullifiers); how value is conserved without
disclosure (value commitments and the binding signature); how a node authorises a spend (RedPallas); and
how a sender privately delivers and a recipient detects a note (note encryption and trial decryption). We then
state the Action relation formally, show the procedure by which a node verifies it without ever seeing the
note, trace the end-to-end security reductions, and close the volume with the Zcash
...
\item \textbf{The four requirements in turn}: notes (\S\ref{sec:notes}), the
commitment tree (\S\ref{sec:tree}), nullifiers (\S\ref{sec:nullifier}), and
value commitments (\S\ref{sec:value}); together with \emph{RedPallas}
...
It is the composition of two stages,
$\GroupHash^{\G}(D, M) := \mathrm{map}_{\G}(\mathrm{hash}(D, M))$, where
$\mathrm{hash}$ maps the input to a base-field element (via BLAKE2s with $D$
as personalisation) and $\mathrm{map}_{\G} : \F_p \to \G$ deterministically
encodes that field element as a curve point. The IETF hash-to-curve standard
fixes the explicit form of $\mathrm{map}_{\G}$ --- the simplified SWU map composed with an isogeny;
...
That last property is the purpose of the construction: because both stages
are public and deterministic, the public strings $D, M$ pin down
...
so it inherits the same hardness assumption as everything else in Orchard
rather than introducing a new one. It builds directly on the preliminaries of
...
\[
\mathsf{SinsemillaCommit}_r(D, M) := \Sinsemilla_D(M) \dotplus [r]\,H_D \;\in\; \G,
\]
where $r \in \F_{p_{\Vesta}}$ is the commitment randomness (trapdoor) and
$H_D \in \G$ is a fixed blinding generator for the domain $D$, derived as
the $\GroupHash$ output
\[
H_D := \GroupHash^{\G}\bigl(D,\ \mathtt{r}\bigr) \in \G
\]
(a nothing-up-my-sleeve point, \S\ref{sec:prelim-curve}). Because $H_D$ is
a $\GroupHash$ output, no party knows a discrete-log relation between it and
the generators $Q(D), P[m]$ that $\Sinsemilla_D$ uses; this independence is
exactly what makes the $[r]H_D$ term a hiding blinding factor whose presence
no one can forge, the same role the independent base $H$ plays in a Pedersen
commitment (the \emph{Crypto Guide}). The \emph{short commitment} returns only
...
\]
with $\Extract$ the coordinate map of \S\ref{sec:prelim-curve}.
\end{definition}
...
$\mathtt{z.cash:Orchard\text{-}NoteCommit}$. The short form $\ShortCommit$
appears wherever the consumer requires a field element instead of a point ---
the Merkle-tree node hash (\S\ref{sec:tree}) and the incoming viewing key
$\ivk$ (\S\ref{sec:orchard-keys}). When we write $\ShortCommit$ with two
message arguments, as in $\ShortCommit_{\rivk}(\aksym^\star, \nksym)$, the
message $M$ is their bit-string concatenation $\aksym^\star \,\|\, \nksym$;
for $\ivk$ the domain is $\mathtt{z.cash:Orchard\text{-}CommitIvk}$ and the
result lies in $\{1, \dots, p_{\Pallas}-1\}$ (we exclude the zero output, which
cannot occur for a valid key, so that $\ivk$ is a usable
scalar).
...
\emph{Expansion function.} The expansion uses BLAKE2b, a conventional
(non-arithmetisation-friendly) cryptographic hash; ``$\mathrm{BLAKE2b\text{-}512}$''
denotes the instance with a $512$-bit ($64$-byte) digest. BLAKE2b natively
supports a \emph{keyed} mode: it takes an optional secret key
$K \in \{0,1\}^{\le 512}$ alongside the message $m$ and computes a
keyed digest $\mathrm{BLAKE2b\text{-}512}(K, m)$ (internally, it pads the key
to a full block and prepends it to the message), so that without
knowledge of $K$ the output is pseudorandom. This is why we write the function
with two arguments: the first slot is the key, the second the
message. Using it in keyed mode is what converts a plain hash into a
\emph{pseudorandom function} (PRF) --- a keyed family
$\{f_K\}_K$ whose outputs are, for a secret uniform $K$, computationally
indistinguishable from a truly random function of the message.
...
modulus is just over $2^{254}$ (about $2^{254}$, of bit length $255$, hence under $2^{255}$), the reduction is statistically
...
\mathtt{G\text{-}})$ (a nothing-up-my-sleeve point, \S\ref{sec:sinsemilla}).
...
\begin{align*}
\ivk &:= \ShortCommit_{\rivk}\bigl(\aksym^\star,\ \nksym\bigr) \in \{1, \dots, p_{\Pallas}-1\} \subset \F_{p_{\Pallas}}, \\
K &:= \mathrm{I2LEBSP}_{256}(\rivk), \\
...
\end{align*}
where $\ShortCommit$ is from Definition~\ref{def:sinsemilla-commit}. This
...
afterwards from $\dk$. Here $\aksym^\star$ is the $\star$-encoding of
$\aksym$ (\S\ref{sec:prelim-curve}), while $\mathrm{I2LEOSP}_{256}$ denotes
...
spendable address for the same $\ivk$. No derivation fixes it
and the wallet must avoid no value --- it allocates indices on demand,
...
invoice to keep their addresses mutually unlinkable; selecting indices
uniformly at random is equally valid and avoids leaking a count. Every
index works for a reason specific to Orchard: the hash-to-curve
$\GroupHash^{\Pallas}$ of \S\ref{sec:prelim-curve} is the simplified-SWU map,
which returns a valid point for every input, so $\gd$ is always a
well-defined group element (and never $\mathcal{O}$, as the Pallas group has
prime order). This is a deliberate improvement over Sapling, where the
...
encryption), not a relabelling, for three reasons that no simpler map
satisfies together. (i) \emph{Unlinkability:} the wallet hands out indices in
order $0, 1, 2, \dots$, so if $d = d_{\mathrm{plain}}$ the diversifiers in a
wallet's addresses would be consecutive integers --- revealing that they
share an owner and how many it has issued; encrypting under $\dk$ scatters
them pseudorandomly over $\{0,1\}^{88}$, so each $d$ looks uniform and
uncorrelated. (ii) \emph{Bijection:} a permutation never collides two indices
onto one $d$, and is \emph{invertible} --- with $\dk$ the wallet decrypts $d$
back to its index, so it can re-derive any address from the seed and recover
which index an address used, storing no per-address secret. A plain hash
would be neither collision-free nor invertible. (iii)
\emph{Format-preserving:} the address format fixes the diversifier at exactly
$88$ bits, and FF1 maps $\{0,1\}^{88} \to \{0,1\}^{88}$ bijectively, which a
truncated hash cannot. AES-256 is merely the standard block cipher
underlying NIST FF1; this derivation runs \emph{wallet-side, never
in the circuit}, so --- unlike the note commitment and Merkle hash, which must
be SNARK-cheap (Sinsemilla, Poseidon) --- a conventional fast cipher is
appropriate here, requiring no arithmetisation-friendliness.
\end{definition}
...
\begin{tikzpicture}[
  ...
  op/.style={font=\footnotesize, text=black!65, anchor=west, align=left, text width=52mm},
...
% ak, nk, rivk bundle into fvk; nk and rivk point at their own slot in the fvk tuple
\coordinate (fvknk)   at ($(fvk.north)+(0.15,0)$);
\coordinate (fvkrivk) at ($(fvk.north)+(1.25,0)$);
...
\node[op] at (\gx,-7.8)  {$\ivk := \ShortCommit_{\rivk}(\aksym^\star,\nksym)$};
\node[op] at (\gx,-9.4)  {$(\dk,\ovk) := $ the two halves of\\[1pt]
                          $\mathsf{PRFexpand}_{\rivk}(\mathtt{0x82}\,\|\,\aksym\,\|\,\nksym)$};
\node[op] at (\gx,-10.6) {$\gd := \GroupHash^{\Pallas}(\mathtt{Orchard\text{-}gd},d)$,\\[1pt]
                          $\pkd := [\ivk]\,\gd$;\\[1pt]
...
bottom. Each arrow is a one-way derivation; the right-hand gutter shows the
operation that produces the child from its parent(s). The $\dk$ and
...
\emph{Every node is secret key material (shaded) except the
diversified address (unshaded), the only value the protocol ever publishes} ---
...
handle anyone can pay; one $\ivk$ generates unboundedly many unlinkable
addresses, all spendable by the same key.
\end{itemize}
...
confers on the recipient the ability to deanonymise every payment to or from the
wallet, so the owner shares it only deliberately (with an auditor, say) and
otherwise guards it like any other secret. The ladder is useful precisely
...
a commitment, $\ShortCommit_{\rivk}(\aksym^\star, \nksym)$, rather than a plain
hash?} Because $\ivk$ must bind \emph{both} $\aksym$ (spend authority) and
$\nksym$ (nullifier derivation) to one identity: a commitment lets the
in-circuit spender prove $\pkd = [\ivk]\gd$ \emph{and} that the same $\ivk$
ties to the $\aksym$ and $\nksym$ used elsewhere in the same proof (checks
A5--A7 below), which a one-way hash would not permit it to open algebraically.
...
$\rho \in \F_{p_{\Pallas}}$ is a per-note uniqueness anchor;
$\psi \in \F_{p_{\Pallas}}$ is sender-chosen randomness; and
...
\pkd^\star \in \{0,1\}^{255}$. The $\NoteCommit$ input is the bit-string
...
This is the canonical use case for a Merkle tree as a privacy set. The
network maintains one append-only tree whose leaves are \emph{every note
commitment ever created}, in order. To prove that her note is valid, Alice
proves \emph{membership}: she exhibits a Merkle authentication path from
her leaf $\cmx$ up to a root the network already trusts. The proof reveals
that her note is \emph{one of} the leaves, but a Merkle path inside the
zero-knowledge circuit hides \emph{which} leaf --- the verifier checks the
path arithmetic without seeing the position or the sibling values. The
anonymity set is the entire tree: the larger it grows, the better the
privacy.
...
layer $\ell'$ is
\[
\mathsf{MerkleCRH}^{\mathsf{Orchard}}_{\ell'}(\ell, r) := \mathsf{ShortHash}_{\mathtt{Orchard\text{-}MerkleCRH}}(\mathrm{LE}_{10}(\ell') \,\|\, \ell \,\|\, r) \;\in\; \F_{p_{\Pallas}},
\]
a layer-tagged Sinsemilla short hash (so leaves $\cmx$, internal nodes, and
the root all live in $\F_{p_{\Pallas}}$). We call a historic tree root an
\emph{anchor} (a single field element, $\mathsf{anchor} \in
\F_{p_{\Pallas}}$); the network remembers recent anchors, and a spend may
prove membership against any of them.
\end{definition}
...
$2$: the choice is not arbitrary --- $2^3 + 5 = 13$ is a non-square modulo
both Pasta primes (the \emph{Math Guide} verifies this), so $2$ is not the
$x$-coordinate of any curve point, hence can never equal a valid $\cmx$ and
can never be silently confused with a genuine commitment.

\paragraph{Derivation of the authentication path.}
The path is \emph{not} something Alice stored or had to track; it is a
deterministic function of \emph{public} data. The tree's leaves are every
$\cmx$ ever published, in order, and the path is the sibling at each level
on the route from her leaf up to the root (Figure~\ref{fig:merklepath}); and
every one of those siblings is itself a hash of public leaves. Hence anyone holding
the public commitments can \emph{recompute} any note's path, against any past
root, at any time. The only secret is \emph{which} leaf is hers, and that
remains hidden because she proves the path in zero knowledge (check~A3), not
because the leaves are hidden.

Seen this way, a years-old note is no special case. To spend it Alice selects an
\emph{anchor} --- a recent root the network already trusts --- reads the public
commitments that were in the tree at that anchor, and hashes them up to obtain
her siblings. A freshly restored, long-offline wallet performs exactly this: it
rebuilds the path from the chain. (The ``incremental witness'' a
continuously-online wallet maintains as the tree grows is only an optimisation; a
cold wallet recomputes from public data instead.) The wallet performs this
\emph{locally} and never requests a particular note's path from a server, which would
reveal which note is hers.
...
\node[lf,route] (L2) at (2,0) {};
\node[lf,sibl]  (L3) at (3,0) {};
...
order. To prove her leaf (orange) is in the tree, Alice recomputes the bold
route up to the root $R$ --- the \emph{anchor} --- folding in, at each level, the
sibling hash beside it: the \emph{authentication path} $A, L_3, F$ (blue). Every
sibling is itself a hash of public leaves, so Alice recomputes the entire path from
public data and stores nothing. Efficiency is the only reason not to hash
...
The one substantive difficulty is efficiency: the live tree holds up to $2^{32}$
leaves, and Alice wishes neither to download all of them nor to hash the whole
tree for one path. This is unnecessary. Her note resides in a single small subtree (a
``shard'' of $2^{16}$ leaves in Orchard), and she needs only \emph{that} shard's leaves,
to build the siblings nearest her own. Every \emph{other} subtree
--- millions of leaves of no relevance to her --- enters her path through its
single \emph{root} alone (the faded subtree of Figure~\ref{fig:merklepath},
folded into $F$). A light wallet therefore fetches only the precomputed roots of
the other shards ($\mathsf{GetSubtreeRoots}$), the leaves of its own shard, and
a frontier checkpoint, and hashes them into the path. ``Obtaining the path'' is,
end to end, \emph{recomputation of it from public commitment data} --- the protocol
merely serves that data in a form that skips the irrelevant leaves. The result
is the private witness for check~A3; only the roots (anchors) are ever public.

\paragraph{Verification of an anchor as the tree grows.}
An \emph{anchor} is the value of the Merkle root at some past moment ---
a single field element. The apparent difficulty is that the tree is
append-only and continues to grow, so by the time Alice's transaction is
mined the live root has advanced. Three facts make proving against a
fixed anchor both possible and sound:
\begin{itemize}[leftmargin=*]
\item \emph{A root is a fixed commitment to a fixed leaf set.} The root
is a deterministic hash of all leaves present when someone took it. It does
not change retroactively: appending new leaves later produces \emph{new}
roots but leaves every \emph{past} root exactly as it was. Thus ``the
anchor'' unambiguously denotes one historical state of the tree.
\item \emph{A membership path is relative to one root.} Alice's
authentication path proves $\cmx_{\mathrm{old}}$ is a leaf of the tree
\emph{whose root is that anchor}. Because her note arose before anyone took the
anchor, such a path exists and she can compute it. The circuit
(check A3) recomputes the root from her leaf and path and asserts it
equals the public anchor --- collision resistance of the layer-tagged
hash (\S\ref{sec:sinsemilla}) renders forging a path to a root for a
non-member infeasible.
\item \emph{Consensus accepts any historical main-chain root, not just
recent ones.} A node treats a value as a valid anchor once it has appeared
as the Orchard tree root at the end of some block on the node's accepted
main chain. There is \emph{no} short sliding window: an anchor may be
arbitrarily old, the only floor being the Orchard activation height (NU5,
mainnet block $1{,}687{,}104$) --- one cannot anchor to a root that
predates the tree's existence. A spend names which anchor it used; the
node accepts the proof iff that root belongs to a block it accepts. This
fully decouples proving from broadcasting: Alice need not race the chain
tip. The only \emph{recency} caveat runs in the opposite direction --- if she anchors
to a very recent block that a later reorganisation removes, her anchor is no
longer on the main chain and she must rebuild the transaction; anchoring at
least $100$ blocks deep (Zcash's reorg/finality limit) avoids this
entirely.
\end{itemize}
Crucially, proving against an old root is \emph{not} a soundness hole:
membership in an older, smaller tree implies the note was committed even
earlier, which is precisely what Alice must establish. The nullifier
mechanism (\S\ref{sec:nullifier}), not the anchor, is what prevents
spending the same old note twice.
...
$[F_{\nksym}(\rho)] G$ masks the commitment with a value that appears random
...
note takes as its anchor field the nullifier of the note spent alongside it,
...
with $\pkd = [\ivk]\,\gd$ and $\gd = \GroupHash^{\Pallas}(\mathtt{Orchard\text{-}gd}, d)$
(\S\ref{sec:orchard-keys}). She forms the \emph{note plaintext}
$\mathsf{np} = (\mathtt{0x02}, d, v, \mathsf{rseed}, \mathsf{memo})$, where the
$32$-byte $\mathsf{rseed}$ is the master randomness from which she derives the note's $\rcm$,
$\psi$, and the ephemeral secret below. She then runs a one-shot
Diffie--Hellman to a \emph{fresh} key:
\[
\mathsf{esk} \;\text{(derived from } \mathsf{rseed}\text{)}, \quad
...
$C_{\mathrm{enc}} := \mathsf{Enc}_{K_{\mathrm{enc}}}(\mathsf{np})$. Because
$\pkd = [\ivk]\gd$, Bob can reach the same secret from his side as
$[\ivk]\,\mathsf{epk} = [\ivk][\mathsf{esk}]\gd = [\mathsf{esk}]\pkd$, so exactly
the holder of $\ivk$ can recover $K_{\mathrm{enc}}$. A second ciphertext
$C_{\mathrm{out}}$, sealed under a key derived from the sender's $\ovk$, carries
$(\pkd, \mathsf{esk})$ so that the \emph{sender} --- or an auditor holding $\ovk$
--- can re-derive $\mathsf{s}$ and read the note as well. The Action publishes
...
(i)~that he formed the ephemeral key honestly, $[\mathsf{esk}]\gd = \mathsf{epk}$
...
a light wallet does so against \emph{compact} blocks, which carry only the $52$
bytes of $C_{\mathrm{enc}}$ that cover $(d, v, \mathsf{rseed})$, together with
$\mathsf{epk}$ and $\cmx$. The wallet records each note that opens \emph{with its leaf
...
scalar), and $V^{\mathsf{Orchard}}, R^{\mathsf{Orchard}} \in
\Ep(\F_{p_{\Pallas}})$ are independent Pallas generators.
\end{definition}
...
publicly declared net flow $v_{\mathrm{balance}}^{\mathsf{Orchard}}$ (the
amount entering or leaving the shielded pool, in the transaction header).
...
\emph{binding signature} (RedPallas) under key $\mathsf{bvk}$ --- is precisely
proving that the accounts balance, with no amount revealed. A single signature
over the whole transaction additionally binds all its Actions together, so that
...
\emph{key re-randomisation}: given a fresh randomiser
$\alpha \in \F_{p_{\Vesta}}$,
\[
...
key for the underlying authority. The circuit enforces (check A6) that
$\rk$ is a re-randomisation of the $\aksym$ bound into the note's address,
...
ZEC from a note worth $5$ ZEC. The unit of shielded transfer is an
...
\item constructs Bob's new note $\mathbf{n}_{\mathrm{new}} := (\mathsf{addr}_{\mathrm{Bob}}, 5, \rho_{\mathrm{new}}, \psi_{\mathrm{new}}, \rcm_{\mathrm{new}})$
...
$v_{\mathrm{old}} - v_{\mathrm{new}} = 5 - 5 = 0$ (\S\ref{sec:value});
...
authorised to spend it (A6, A7), the new note is well-formed (A1, A2), and
...
and open it), attaches a SpendAuthSig under $\rk$, and signs the whole
bundle with the binding signature under $\mathsf{bvk}$ (\S\ref{sec:value}),
which proves balance and welds the Action into this transaction.
...
the ciphertext, recovers $\mathbf{n}_{\mathrm{new}}$, and now owns a $5$-ZEC
...
on the proof, not on the note. Using only the public fields
$(\mathsf{anchor}, \cvsym^{\mathsf{net}}, \nfsym_{\mathrm{old}}, \rk, \cmx)$ of
each Action, a node checks:
\begin{itemize}[leftmargin=*]
\item the \emph{Halo 2 proof} of the Action statement (\S\ref{sec:action-stmt})
--- this forces the spent note to be a genuine tree leaf, the nullifier
and new commitment to follow correctly, and the keys to align;
...
\end{itemize}
If every Action passes, the node adds each $\nfsym_{\mathrm{old}}$ to the
nullifier set and appends each $\cmx_{\mathrm{new}}$ as a new tree leaf --- the
two state changes that permit the next spender to prove membership and that make a
second spend of the same note fail. Thus the SNARK together with the two signatures
...
\]
and the note, keys, Merkle path, randomiser $\alpha$, and randomness as
private witness, the conjunction:
...
$\cmsym_{\mathrm{new}} = \NoteCommit_{\rcm_{\mathrm{new}}}(\ldots)$ and
$\cmx = \Extract(\cmsym_{\mathrm{new}})$ --- the created note is well-formed
and its anchor chains to the spent note's nullifier (Faerie resistance).
...
note's:
$\nfsym_{\mathrm{old}} = \Extract([F_{\nksym}(\rho_{\mathrm{old}}) + \psi_{\mathrm{old}}] G^{\mathsf{nf}} + \cmsym_{\mathrm{old}})$.
...
$\ivk = \ShortCommit_{\rivk}(\aksym^\star, \nksym)$ and
$\pkd_{\mathrm{old}} = [\ivk] \gd_{\mathrm{old}}$.

\noindent
\textbf{[A8] / [A9] Spend / output gating}:
$v_{\mathrm{old}} \cdot (1 - \mathsf{enableSpends}) = 0$ and
$v_{\mathrm{new}} \cdot (1 - \mathsf{enableOutputs}) = 0$ --- permitting consensus
to disable spends or outputs bundle-wide.
\end{definition}

Each conjunct is one entry from the four-requirement map of
...
value, and A6--A7 authorise the spender. This is the relation
$\Rel_{\mathsf{Orchard}}$; Halo 2 produces one proof of it per Action.
...
for every Action --- supplied the note, keys, Merkle path, randomiser, and
randomness as private witness. We sketch this encoding here; the gate-level
...
rather than a bit-oriented hash.

\paragraph{The ECC gadget.} Implements the Pallas group law with the
\emph{complete} addition formulas (no edge-case failures,
\S\ref{sec:prelim-curve}), together with variable- and fixed-base scalar
multiplication (the former exploiting the $j$-invariant-$0$ endomorphism, the latter using precomputed fixed-base tables) and
...
$\mathrm{sign} \in \{-1,+1\}$ (A4), that point and field encodings are canonical,
and that the selectors $\mathsf{enableSpends}, \mathsf{enableOutputs}$ are boolean
(A8, A9).
...
proved with the Halo~2 inner-product argument over the Pasta cycle. Every Action,
whatever it spends or creates, is one instance of this same circuit, yielding a
constant-size proof that a node checks (\S\ref{sec:netverify}) from the public
inputs alone; the accumulation machinery of the \emph{Halo~2 Guide} then lets
...
(the \emph{Halo 2 Guide}, the \emph{Halo 2 Guide}) of Halo 2, an extractor can
recover this witness from any successful prover. The Orchard security
properties (Theorem-Application chain below) each follow from one or
more conjuncts of A1--A9 plus the algebraic properties of the
primitives.
...
A collision $M \ne M'$ with $\Sinsemilla_D(M) = \Sinsemilla_D(M')$
...
non-trivial linear relation over $\F_q$ on the generators. Since
hash-to-curve modelled as a random oracle samples these, by DL-Rel
(the \emph{Crypto Guide}) no efficient adversary produces such a relation
without solving DL. The incomplete-addition exceptions are themselves
...
gives
$[F_{\nksym}(\rho) + \psi] G + \cmsym = \pm([F_{\nksym}(\rho') + \psi'] G + \cmsym')$,
a non-trivial linear relation among $G, \cmsym, \cmsym'$. Since
$\cmsym, \cmsym'$ are themselves Pedersen-style commitments to distinct
notes with hidden trapdoors, finding such a relation reduces to DL on
Pallas.
\end{proof}
...
model. The re-randomisation extension preserves security: a forger
against $\rk$ for adversary-chosen $\alpha$ implies a forger against
plain Schnorr on $\rk$.
\end{proposition}

\begin{proof}[Sketch]
The standard Pointcheval--Stern forking argument
(the \emph{Crypto Guide}, the \emph{Crypto Guide}) reduces a Schnorr forger
to a DL adversary. For re-randomisation, the security simulator selects
$\rsk$ uniformly, sets $\rk := [\rsk] G^{\mathsf{SpendAuth}}$, then
produces a consistent $(\aksym, \alpha)$ by sampling $\alpha$ at random
and computing $\aksym := \rk - [\alpha] G^{\mathsf{SpendAuth}}$. The
adversary's view of $\aksym$ is uniform regardless of $\rsk$, so the
forging probability against $\rk$ matches that against plain Schnorr
on a uniform key.
\end{proof}
...
except with probability negligible in the DL hardness of Pallas.
\end{proposition}
...
binding signature; by unforgeability (Proposition~\ref{prop:redpallas}),
a valid binding signature implies the verifier-side $\mathsf{bvk}$
equals $[\mathsf{bsk}] R^{\mathsf{Orchard}}$ for some known $\mathsf{bsk}$
--- which forces the value-component to equal
$[v_{\mathrm{balance}}] V^{\mathsf{Orchard}}$, hence the total value flow
matches the declared transparent balance.
\end{proof}
...
(the \emph{Crypto Guide}). The simulator on input
$x := (\mathsf{anchor}, \cvsym^{\mathsf{net}}, \nfsym_{\mathrm{old}}, \rk, \cmx, \ldots)$
operates as follows:
\begin{enumerate}[leftmargin=*]
\item Sample the Fiat--Shamir challenges $\theta, \beta, \gamma, y, x, x_1, x_3$
and the IPA challenges $u_1, \dots, u_k$ uniformly, programming the
...
IPA round) and the polynomial blinders (added in phase 1) to satisfy
...
ZK furnishes Orchard its privacy: the public chain state reveals
only the public inputs of each Action; nothing about the spent note,
recipient, or value beyond the explicit transparent total leaks.

\subsection{Composition of end-to-end soundness}\label{sec:e2e-snd}

The full SNARG soundness composes three ingredients:
\begin{itemize}[leftmargin=*]
...
per check is $\le d_{\max} / |\F| \approx d_{\max} \cdot 2^{-254}$,
overwhelmingly small.
\item \emph{PCS knowledge-binding.} Reduces to DL on the IPA curve
(the \emph{Halo 2 Guide}), with the accumulation analysis
(the \emph{Halo 2 Guide}) preserving soundness across recursion.
\item \emph{Fiat--Shamir soundness in the ROM.} Loses a polynomial
factor in the prover's RO-query budget (the \emph{Crypto Guide}); for
$Q \le 2^{80}$ adversary queries this remains negligible.
\end{itemize}

Each step's soundness error is negligible at Orchard's parameters
($|\F| \approx 2^{254}$, $\lambda = 128$); the union bound across steps
is itself negligible. Combined with the application-level reductions
...
Under DL on Pallas in the random-oracle model, no efficient adversary
can (i) produce an accepted Action whose public inputs do not correspond
to a real spent note, (ii) cause two Actions to share a nullifier
without spending the same note twice, (iii) cause a bundle's value flow
to exceed the transparent balance, or (iv) link a published Action to
any specific recipient or value beyond what is explicit in the public
inputs.
\end{theorem}
...
three reduce (informally) to DL on Pallas in the ROM; the fourth
(privacy) is stronger --- it holds \emph{statistically} given the
perfectly-hiding Pedersen and Sinsemilla blinders and the random-oracle
programming, so it does not rest on the DL assumption at all. This
asymmetry is typical of the discrete-log SNARK family: soundness is
computational (a cheating prover is only \emph{bounded}, not impossible),
whereas zero knowledge is information-theoretic.
...
The original shielded protocol used a \emph{BCTV14}-style SNARK (Ben-Sasson,
Chiesa, Tromer, Virza; USENIX Security 2014), a refinement of the
PGHR13 / Pinocchio QAP-based SNARK family (later superseded by Groth16).
The curve was BN254 (then called Barreto--Naehrig), a pairing-friendly
curve originally believed to provide $\sim 128$-bit security but reduced to
$\sim 100$-bit by the 2016 extended-tower number field sieve advances.

Properties:
\begin{itemize}[leftmargin=*]
\item Trusted setup via a per-circuit Powers of Tau ceremony --- the
original Zcash ceremony of 2016, run as a multi-party computation with
public verification.
\item Constant-size proofs ($\sim 290$ bytes), constant verifier time.
\item Subject to several minor cryptographic debt items (cofactor handling,
edge cases in pairing acceptance) addressed in later iterations.
\end{itemize}

Sprout was the first deployed shielded-transaction system and demonstrated that
zk-SNARK-based privacy was feasible at scale, but its trusted setup,
slow prover, and integer-arithmetic SHA256 used inside the circuit rendered
it a transitional design.
\subsection{Sapling (2018--present)}

Sapling (network upgrade 1) replaced Sprout's stack with:
\begin{itemize}[leftmargin=*]
\item Groth16 (Groth; EUROCRYPT 2016): a SNARK with $192$-byte proofs and
three-pairing verification.
\item BLS12-381: a pairing-friendly curve targeting $\sim 120$-bit security
with a base field large enough to support efficient arithmetic.
\item Jubjub: a Twisted Edwards curve embedded inside BLS12-381's scalar
field, which serves in-circuit elliptic-curve operations (RedJubjub signatures,
ECC-based hashes).
\item Bowe--Hopwood Pedersen hash for the Merkle tree CRH, a bit-by-bit
Pedersen-based construction.
\end{itemize}

Sapling addressed many of Sprout's deficiencies: a more efficient prover (the
Sapling proving time on commodity hardware is several seconds, reduced from
minutes for Sprout), a smaller proof, and improved in-circuit hashing. It
retained, however, per-circuit trusted setup (the Sapling Powers of Tau, run as a
multi-party ceremony with many participants), and its proving system
admitted no efficient recursion (no known elliptic-curve cycle
extends BLS12-381 / Jubjub).

\subsection{Orchard (2021--present, NU5)}
...
\item Sinsemilla hash: a Pedersen-Hash-like algebraic hash optimised for
PLONKish circuits with lookups; slower than Poseidon \emph{inside} a
circuit but substantially faster \emph{outside}, which matters for chain
sync where most clients do not generate proofs.
\item Poseidon: serves select Orchard primitives where in-circuit speed
dominates (the nullifier PRF). The incoming viewing key is instead derived
via CommitIvk, a Sinsemilla-based short commitment, not Poseidon. Its security rests on
cryptanalytic confidence in algebraic-attack resistance rather than on a
hardness assumption such as DL.
\end{itemize}
...
Trusted setup & Per-circuit & Per-circuit (multi-party) & None \\
Recursion-ready & No & No & Yes (via Pasta cycle) \\
Proof size & \textasciitilde 290 B & 192 B & \textasciitilde 1.3--3 KB \\
Verification time & Constant (pairing) & Constant (3 pairings) & $O(\log d)$ amortized \\
\bottomrule
\end{tabular}
\caption{Three generations of Zcash shielded-payment proof systems.}\label{tab:zcash-history}
\end{table}

The proof size grew between Sapling and Orchard; this is the cost of
transparency. The benefit, beyond the removal of trusted setup, is the
