...
succeeds (\S\ref{sec:noteenc}). A checkpoint stores merely the number of
leaves $n$ present at that block --- the size that fixes the block's anchor;
the anchor itself is recomputed from the retained nodes on demand, never
stored.
...
reproduces the path against any block it has checkpointed. Because the
flanking siblings are \emph{complete-subtree roots}, the wallet never stores
or rescans their leaves; a freshly restored wallet fetches those roots
directly --- one $32$-byte root per $2^{16}$-leaf subtree
($\mathsf{GetSubtreeRoots}$) --- and scans leaf by leaf only the subtree that
holds its own note.
