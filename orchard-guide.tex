...
$32$-byte $\mathsf{rseed}$ is the master randomness from which she derives the
...
\begin{align*}
\psi   &:= \mathrm{ToBase}\bigl(\mathsf{PRFexpand}_{\mathsf{rseed}}(\mathtt{0x09} \,\|\, \rho)\bigr), \\
\rcm   &:= \mathrm{ToScalar}\bigl(\mathsf{PRFexpand}_{\mathsf{rseed}}(\mathtt{0x05} \,\|\, \rho)\bigr), \\
\mathsf{esk} &:= \mathrm{ToScalar}\bigl(\mathsf{PRFexpand}_{\mathsf{rseed}}(\mathtt{0x04} \,\|\, \rho)\bigr),
\end{align*}
so $\psi \in \F_{p_{\Pallas}}$ while $\rcm, \mathsf{esk} \in \F_{p_{\Vesta}}$ (the
last taken nonzero). She then runs a one-shot
