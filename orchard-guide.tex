...
Check~A3 reverses the construction: starting from $\cmx$ it folds in one path
entry per level, on the side $b_i$ dictates, and asserts that the final value
equals the public anchor; the bits $b_i$ and the siblings stay inside the
witness.

Each entry is one of two kinds. When $b_i = 1$ the sibling subtree lies to
the \emph{left} of her leaf: it spans commitments older than hers, every one
of its positions is filled, and --- because the tree appends only on the
right --- its root is final and identical at every anchor. When $b_i = 0$ the
...
In practice the wallet computes the path incrementally rather than from
scratch. While scanning the chain it appends every published $\cmx$ --- its
own and everyone else's --- to a local copy of the tree. The moment its note
enters at position $\mathsf{pos}$, all left siblings are determined; the
wallet stores them once and for all. From then on each block's appends
advance only the right siblings: whenever a right sibling subtree fills
completely the wallet records its now-final root, and the single
partially-filled one is recomputed on demand from the tree's
\emph{frontier} --- the roots of the maximal complete subtrees along the
right edge, at most one per level, the compact state from which an
append-only tree is grown and its root computed. Snapshotting this witness
at block boundaries lets the wallet emit her path against the anchor of any
recent block.
...
commitments. Nor must it touch all $2^{32}$ positions. Her note resides in
one \emph{shard} of $2^{16}$ leaves, and only that shard's leaves enter her
siblings at the low levels; every other shard enters her path through its
root alone (the faded subtree of Figure~\ref{fig:merklepath}, folded into
$F$). A light wallet therefore fetches the other shards' precomputed roots
($\mathsf{GetSubtreeRoots}$), its own shard's leaves, and a frontier
checkpoint. It performs the computation \emph{locally}, never asking a server
for one particular note's path, which would reveal which note is hers. The
finished path is the private witness of check~A3; only the anchor is public.
...
entirely. Nor is an old anchor a soundness hole: membership in an older,
