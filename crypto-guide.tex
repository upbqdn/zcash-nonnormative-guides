...
Pallas and Vesta satisfy criteria (1)--(4) of Definition~\ref{assumptions:def:secure-curve} at \(\lambda \approx 126\); criterion (5) fails for Pallas but is rendered moot by mandatory point validation.
\end{proposition}
...
\]
\(N\) is a nonce, \(\mathrm{ad}\) is associated data authenticated but not encrypted, and \(\bot\) denotes rejection. Correctness: \(\mathsf{Dec}_K(N, \mathrm{ad}, \mathsf{Enc}_K(N, \mathrm{ad}, m)) = m\). Security is \emph{authenticated-encryption security} (Definitions~\ref{symmetric:def-intctxt} and~\ref{symmetric:def-ae}): ciphertexts are indistinguishable from random under chosen-plaintext attack \emph{and} no adversary can produce a fresh ciphertext that decrypts to anything but \(\bot\) (ciphertext integrity), so confidentiality and integrity hold simultaneously.
\end{definition}
