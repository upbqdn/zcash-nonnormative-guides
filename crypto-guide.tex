...
Modelling the generator-derivation map \(m \mapsto S(m)\) as producing independent uniform generators (a ``nothing-up-my-sleeve'' hash-to-curve), any collision in \(\mathrm{SinsemillaHash}\) yields a nontrivial discrete-log relation among the generators \(\{S(m)\}\), and hence--via the same every-generator embedding as Proposition~\ref{commitments:prop:pedersen-hash-cr}--a discrete logarithm. Thus Sinsemilla is collision resistant under the discrete logarithm assumption in \(\G\).
\end{proposition}

\begin{proof}
Sketch. Two equal-length messages \(M \neq M'\) colliding equate two combinations of the unrolled form; subtracting, the constant \([2^k]Q\) cancels and one obtains
\[
  \sum_{j=1}^{k} \bigl[2^{\,k-j}\bigr]\bigl( S(m_j) - S(m'_j) \bigr) \;=\; \mathcal{O},
\]
a nontrivial integer relation among finitely many distinct table generators: the messages differ in some position \(j\), there \(S(m_j) \neq S(m'_j)\), and the bounded coefficients \(\pm 2^{\,k-j}\) do not vanish modulo the group order for admissible message lengths (a finite, design-time check). Embedding a DLog challenge in every table generator, exactly as in Theorem~\ref{commitments:thm:vector-props}, recovers the challenge logarithm from the relation.
\end{proof}
