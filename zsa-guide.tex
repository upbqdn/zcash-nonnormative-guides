...
\thispagestyle{empty}
\begin{abstract}
Assuming the Orchard protocol of the \emph{Ironwood Guide}, this volume of
...
cited, never re-derived: notes and note commitments (\emph{Ironwood Guide},
...
the binding signature (\emph{Ironwood Guide}, \S``Conservation of value: value
commitments and the binding signature''); the Action statement and dummy
spends (\emph{Ironwood Guide}, \S``The Action statement, formally'');
nullifiers (\emph{Ironwood Guide}, \S``Prevention of double-spending:
nullifiers''); Sinsemilla (\emph{Ironwood Guide}, \S``The commitment primitive:
...
omitted when clear from context (\texttt{zip-0227.rst}:224). The
...
the isogeny --- the \emph{Ironwood Guide} develops in ``Preliminaries:
...
in the \emph{Ironwood Guide}, ``Conservation of value: value commitments
...
\emph{Ironwood Guide}, \S``Transmission and detection of a note'');
...
construction it modifies in the \emph{Ironwood Guide} rather than re-deriving
...
$(\mathsf{addr}, v, \rho, \psi, \rcm)$ of the \emph{Ironwood Guide},
...
\]
the fixed value-commitment base of the \emph{Ironwood Guide}, ``Conservation
...
baseline layout of the \emph{Ironwood Guide}, ``Representation of a note'':
\[
...
exactly as in Orchard (protocol specification \S4.16; \emph{Ironwood Guide},
...
\emph{Ironwood Guide}, ``Conservation of value'', and we do not restate them.
...
the \emph{Ironwood Guide}, ``The Action statement, formally''). For Custom
...
\emph{Ironwood Guide}'s Definition ``Orchard Action statement'' exists in any
...
the baseline is the \emph{Ironwood Guide}, ``Conservation of value'', its
...
Action pads with no real input'' (the \emph{Ironwood Guide}, ``The Action
...
$V^{\mathsf{Orchard}}$ (\emph{Ironwood Guide}, ``Conservation of value:
...
$R^{\mathsf{Orchard}}$ the randomness base of the \emph{Ironwood Guide}'s
...
against the Action statement A1--A9 of the \emph{Ironwood Guide} (``The
...
\end{equation}
in the notation of the \emph{Ironwood Guide}'s A5 (``The Action statement,
...
\S\ref{sec:zsa-tx-rules}). A normative rule of ZIP-230 (its consensus-rule
list, not the rationale) states ``In NU7, \texttt{nExpiryHeight} MUST be set
to 0'' --- read literally, a constraint on
...
in the \emph{Ironwood Guide}, \S``Conservation of value: value commitments and
...
\item \emph{Lead byte.} The implementation hard-codes
  ...
  lead byte an unassigned placeholder. The fork's choice predates upstream
  ZIP-2005 assigning $\mathtt{0x03}$ to Ironwood notes and directing future
  proposals to pick a new value; the collision is fork-only context, with no
  format ambiguity on Zcash mainnet. Reassignment is a hard prerequisite
