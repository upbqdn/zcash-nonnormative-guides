...
\thispagestyle{empty}
\begin{abstract}
This volume of the series --- \emph{Math Guide}, \emph{Crypto Guide},
\emph{Halo~2 Guide}, \emph{Orchard Guide}, \emph{Wallet Guide}, \emph{Voting
Guide}, \emph{Tachyon Guide} --- documents a protocol that does not yet exist.
...
\emph{Orchard Guide} cites shipped code and a final specification. Tachyon has
...
tree (the \emph{Orchard Guide}, \S``Transmission and detection of a note:
...
Section~\ref{sec:tachyon-dbu-nf} constructs the derivation in full --- the
climb and leaf domains, and the nowhere-fixed depth $D$ and arity $A$.
...
(\emph{Orchard Guide}, \S``Transmission and detection of a note: encryption,
...
on demand from retained subtree roots (\emph{Orchard Guide}, \S``Proof of
...
(\emph{Orchard Guide}, \S``What a node verifies, without ever observing the
...
protocol answers with in-band secret distribution (\emph{Orchard Guide},
...
deterministic per spending key (book, \texttt{src/keys.md} at
\texttt{26fdc165}, which opens with the in-file warning ``this is
...
(nowhere fixed; \S\ref{sec:tachyon-dbu-epoch}); the Poseidon instances,
...
fixed nowhere in the book or crate. The ePrint note supplies only the
...
splice identities of the form $C(X) = L(X) + X^{s}(p-1) + X^{s+1}R(X)$
...
The post-quantum analysis of this series (the \emph{Orchard Guide},
\S``Post-quantum security of the deployed protocol'') reaches one headline
