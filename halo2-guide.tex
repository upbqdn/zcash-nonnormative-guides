...
analysis of BCMS20, by contrast, does not invoke the AGM: it proves
...
each invocation costs $1/p^*$ expected queries to the prover, yielding
total cost $O(d^{\log_2 3} \cdot \mathsf{poly}(\lambda) / p^*)$.
...
elements ($\cmsym$, $\rk$, $\cvsym^{\mathsf{net}}$, the accumulator's
$G^{(0)}$) have coordinates in exactly that field; the next Pallas proof
...
single combination $q_i(X)$ by powers of $x_1$. For each group, both
...
\emph{only} when the advice cells holding $\nfsym$,
$\cvsym^{\mathsf{net}}$, $\rk$, and the flags equal the public values the
