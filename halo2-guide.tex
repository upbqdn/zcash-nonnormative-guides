...
proofs at once (\S\ref{sec:halo-acc}).

\subsection{The algebraic group model}

The IPA's knowledge-soundness proof proceeds through the tree extractor of
\S\ref{sec:ipa-detail}, with knowledge error $O(d / |\F|)$. Obtaining the
tree of accepting transcripts in the standard model by iterated forking,
however, is loose: each of the $\log d$ rounds contributes a
multiplicative blow-up, so the standard-model reduction to DL runs in
quasi-polynomial time $d^{O(\log d)}$ rather than strict polynomial time.
...
disambiguates every occurrence.
...
Extracting the full witness $\vec{a} \in \F_p^d$ requires three
successful transcripts at the deepest level, then for \emph{each triple},
three transcripts at the next-shallower level, and so on. The tree has
$3^k = d^{\log_2 3}$ leaves --- polynomial in $d$ (\S\ref{sec:ks}) ---
yielding polynomial expected extraction time when $d = \poly(\lambda)$.
The knowledge error of $O(d / |\F_p|)$ accounts for the probability of
challenge collisions (e.g.\ $u_j = \pm u_j'$) anywhere in the tree, which
a union bound renders negligible.
...
$H \in \G$. The commitments to the fixed, selector, and permutation
