...
If $\Pi$ has knowledge soundness $\kappa_{\Pi}$ and the PCS is extractable
...
pairwise distinct ($u_j \ne \pm u_j'$). Excluding the bad challenge collisions adds
...
the quotient pieces the same way; Remark~\ref{rem:active-rows} describes
how the constraint system exempts the blinding rows, so the random values
never violate an argument.
