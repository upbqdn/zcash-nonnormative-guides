...
This is the \emph{Ironwood Guide}, the fourth and final volume of a development of the cryptography
...
(the general-purpose proof system). This volume assembles those tools into the Orchard
protocol as carried by its current pool, \emph{Ironwood}. ZIP-229 fixes the
vocabulary up front: the \emph{Orchard protocol} is the shared cryptographic
construction --- curves, Sinsemilla, the Action circuit, notes, commitments,
nullifiers, keys, and note encryption --- and it survives; the \emph{Orchard
pool} is the legacy value pool it ran from NU5, sealed to spend-only at
NU6.3; the \emph{Ironwood pool} is the current pool (specified; activated on
Testnet at height $4134000$; Mainnet activation pending --- NU6.3, ZIP-258,
Draft, height TBD at \texttt{zips} commit \texttt{69610984}).
...
proof-system lineage from Sprout through Sapling to Orchard, and within
Orchard the three circuit versions ending at Ironwood.
...
Orchard activation onward (NU5, mainnet block $1{,}687{,}104$) --- for the
legacy Orchard-pool tree: the two pools keep separate, independent
commitment trees and nullifier sets (ZIP-229), and an Ironwood Action
instead proves against $\mathsf{anchorIronwood}$ over the Ironwood tree,
empty at NU6.3 activation (Testnet height $4134000$, Mainnet TBD; ZIP-258),
whose anchors are valid from activation onward. In neither pool is there a
...
$\mathsf{np} = (\mathtt{0x03}, d, v, \mathsf{rseed}, \mathsf{memo})$, where the
...
derivation is one $\mathsf{PRFexpand}$ call keyed by $\mathsf{rseed}$ and
distinguished by a leading domain-separator byte; $\mathsf{esk}$ and $\psi$
take $\rho := \nfsym_{\mathrm{old}}$ (as $32$ little-endian bytes) as input
--- which binds the note's secret randomness to its Action --- and $\rcm$,
derived last, takes every note field:
\begin{align*}
...
\rcm   &:= \mathrm{ToScalar}\bigl(\mathsf{PRFexpand}_{\mathsf{rseed}}(\mathsf{pre\_rcm})\bigr) \in \F_{p_{\Vesta}}, \\
\mathsf{pre\_rcm} &:= \mathtt{0x0B} \,\|\, \gd^\star \,\|\, \pkd^\star \,\|\, \mathrm{LE}_{64}(v) \,\|\, \mathrm{LE}_{256}(\rho) \,\|\, \mathrm{LE}_{256}(\psi),
\end{align*}
where $\mathsf{esk}$ is redrawn under a fresh $\mathsf{rseed}$ in the rare event
it vanishes. This is the Ironwood derivation, the current note format
(ZIP-2005; \S\ref{sec:iw-note} gives the exact byte layout and the order's
rationale --- $\rcm$ comes last because its input includes $\psi$); legacy
lead-byte-$\mathtt{0x02}$ notes --- all Orchard-pool notes --- instead
derive
$\rcm := \mathrm{ToScalar}(\mathsf{PRFexpand}_{\mathsf{rseed}}(\mathtt{0x05}
\,\|\, \rho))$ (Remark~\ref{rem:iw-legacy-rcm}). She then runs a one-shot
...
plaintext: the lead byte ($1$ byte; $\mathtt{0x03}$ for Ironwood outputs,
the only allowed value in that pool --- $\mathtt{0x02}$ appears only when
scanning legacy Orchard-pool outputs), $d$ ($11$ bytes), $v$
...
flowing \emph{out} of the pool --- paying fees or transparent
outputs, say).
...
no one can remove or transplant any of them. From NU6.3 the construction
runs once per pool: a v6 transaction carries independent
$v_{\mathrm{balance}}^{\mathsf{Orchard}}$ and
$v_{\mathrm{balance}}^{\mathsf{Ironwood}}$ fields, and each pool's Actions
sum to their own $\mathsf{bvk}$, checked by their own binding signature over
the same bases $V^{\mathsf{Orchard}}, R^{\mathsf{Orchard}}$ (ZIP-229;
\S\ref{sec:iw-pools}).
...
The v6 form of the digest appends the Ironwood component digest as a fifth
child, $d_{\mathrm{Irw}}$, under revised personalisations, and moves the
Sapling, Orchard, and Ironwood anchors from effecting data to authorizing
data (\S\ref{sec:iw-pools}; ZIP-229; \texttt{librustzcash}
\texttt{zcash\_primitives}, \texttt{transaction/sighash\_v6.rs}).
...
What the node receives is an Orchard-protocol \emph{bundle}, the unit into
which the transaction format groups one pool's data: the per-Action
...
$\mathsf{enableOutputs}$, and --- from NU6.3 ---
$\mathsf{enableCrossAddress}$ (bit~2 of the flags byte, reserved as $0$ in
v5 transactions before NU6.3) are likewise bundle-level fields. The current
carrier is the v6 format (ZIP-229): the v5 format's Orchard component
(ZIP-225) plus a separate Ironwood component holding that pool's bundle
...
\S\ref{sec:iw-pools}). Transaction versions v4, v5, and v6 all remain valid
from NU6.3 onward, and the Orchard-pool sealing rules of
\S\ref{sec:iw-pools} bind regardless of version (ZIP-229). Using only the
public fields
...
\begin{definition}[Orchard Action statement, Ironwood circuit (NU6.3)]\label{def:action}
The Action SNARK proves, with public inputs
\[
(\mathsf{anchor},\, \cvsym^{\mathsf{net}},\, \nfsym_{\mathrm{old}},\, \rk,\, \cmx,\, \mathsf{enableSpends},\, \mathsf{enableOutputs},\, \mathsf{disableCrossAddress})
\]
...
(ZIP-229). Spending Ironwood notes is otherwise permitted; the
$\mathsf{enableSpends} = 0$ requirement is coinbase-scoped, not a
pool-wide prohibition (protocol specification \S~7.1, transaction consensus
rules).
...
$\pkd_{\mathrm{new}} = \pkd_{\mathrm{old}}$ as full curve points, enforced
by four product constraints of the A3 shape, one per affine coordinate
(Definition~\ref{def:iw-action}, \S\ref{sec:iw-cross}, gives the rows, the
instance index --- the flag is the tenth public input, at index $9$ ---
and the wire encoding). Consensus requires every post-NU6.3 Orchard-pool
Action to set the flag (\S\ref{sec:iw-pools}).
\end{definition}

\begin{remark}[Three circuit versions, one owning ZIP
pending]\label{rem:action-versions}
The relation above is the third Action-circuit version: the deployed
implementation distinguishes the insecure NU5 original, the NU6.2
correction (ZIP-257, Final), and the NU6.3 Ironwood circuit, and only the
last constrains the flag
(\texttt{OrchardCircuitVersion::supports\_cross\_address\_restriction},
crate \texttt{orchard} at \texttt{bef8a27} \texttt{src/circuit.rs}). The legacy NU5/NU6
statement is the relation A1--A9 alone, over the nine-element public-input
tuple ending at $\mathsf{enableOutputs}$. Consensus selects the verifying
key by branch (ZIP-258), so from NU6.3 every Action --- in either
pool, whatever the transaction version --- verifies against the Ironwood
key. The circuit change's owning ZIP does not yet exist as text: ZIP-2006
is a Reserved stub, and the flag's encoding, its consensus rule, and the
verifying-key selection are \emph{specified} in ZIP-229/258
(\S\ref{sec:iw-cross} states what remains reconstructable only from the
implementation).
%% TODO: re-cite ZIP-2006 when published (currently Reserved stub;
%% circuit spec lives in crate orchard)
\end{remark}
...
value, and A6--A7 authorise the spender; A8--A10 stand outside the map as
...
conjunction A1--A10 as one such circuit --- the \emph{Action circuit}, identical
...
canonical. (The flags $\mathsf{enableSpends}$, $\mathsf{enableOutputs}$, and
$\mathsf{enableCrossAddress}$ (bit~2) are verifier-supplied public inputs
--- ZIP-229's flags bytes, $\mathsf{flagsOrchard}$ and
$\mathsf{flagsIronwood}$, fix them to bits outside the circuit --- and enter
the circuit only through the plain-gate products of A8--A10, the third
negated as the Ironwood circuit's tenth public input
$\mathsf{disableCrossAddress}$, \S\ref{sec:iw-cross}.)
...
witness satisfying A1--A10. By knowledge soundness
...
follow from one or more conjuncts of A1--A10 plus the algebraic properties
...
A1--A10 --- in particular, any Action whose witnessed $v_{\mathrm{old}}$ is
...
\subsection{Orchard (2022--present, NU5; Ironwood pool from NU6.3)}
...
The stack has since carried one correction and one split, neither touching
its cryptography. The Action circuit exists in three versions --- NU5's
original, the NU6.2 emergency correction (ZIP-257, Final), and the NU6.3
Ironwood circuit with the cross-address input (\S\ref{sec:iw-cross}) ---
named \texttt{InsecurePreNu6\_2}, \texttt{FixedPostNu6\_2}, and
\texttt{Ironwood} in crate \texttt{orchard} at \texttt{bef8a27}
(\texttt{src/circuit.rs}). And from NU6.3 the one protocol carries two value
pools, the sealed Orchard pool and the current Ironwood pool (ZIP-229,
Terminology; \S\ref{sec:iw-pools}). The Halo~2 / Pasta stack is unchanged
throughout.
...
Guide}). The Orchard column covers three circuit versions (the NU5
original; the NU6.2 correction, ZIP-257, Final; the NU6.3 Ironwood circuit
with the cross-address input) and, from NU6.3, two value pools
(\S\ref{sec:iw-pools}).}\label{tab:zcash-history}
\end{table}
...
bytes in the v5/v6 encodings, including the $2\log_2 2^{11} = 22$ group
...
\section{The Ironwood pool}\label{sec:iw}

The NU6.3 network upgrade (ZIP-258, Draft) closes the story this volume has
told so far and opens its successor: it seals the value pool that has carried
Orchard payments since NU5 and starts a second pool --- \emph{Ironwood} ---
running the same cryptography. ZIP-229 (Draft) fixes the vocabulary this
section uses throughout. The \emph{Orchard protocol} is the shared design:
the Pasta curves, Sinsemilla, the Action circuit (as modified by ZIP-2006),
the note, commitment, nullifier, and key constructions, and the note
encryption (as modified by ZIP-2005) --- everything the preceding sections
constructed. The \emph{Orchard pool} and the \emph{Ironwood pool} are two
value pools of that one protocol, each with its own note commitment tree,
anchor, and chain value pool balance. Nothing in the preceding sections is
discarded; what changes is which pool new value may enter, and how an
Ironwood note derives its trapdoor.

As in \S\ref{sec:pq-orchard}, claims about deployed behaviour cite the
implementation and the specification --- here crate \texttt{orchard} at
\texttt{bef8a27} (the unreleased 0.14.0 dev tree carrying the Ironwood work;
\texttt{librustzcash} itself pins the published \texttt{orchard}~0.15.0 crate),
\texttt{librustzcash} at
commit \texttt{e30517e4}, and \texttt{protocol.tex} and the ZIPs at
\texttt{zips} commit \texttt{69610984} --- and forward-looking claims carry
one of the three epistemic bins \emph{specified}, \emph{designed but
unspecified}, or \emph{open problem}.

\subsection{Two pools, one protocol: activation and framing}\label{sec:iw-pools}

\paragraph{Activation.} NU6.3 carries consensus branch identifier
$\mathtt{0x37A5165B}$ and activates on Testnet at height $4134000$; the ZIP
leaves the Mainnet height TBD, while \texttt{librustzcash} already hardcodes
$3428143$ (\texttt{zcash\_protocol/src/consensus.rs}) --- an
implementation ahead of its specification, which we flag rather than resolve
(ZIP-258).
%% TODO: reconcile the Mainnet ACTIVATION_HEIGHT (ZIP-258 says TBD;
%% librustzcash consensus.rs:496 says 3,428,143) before the volume ships.
%% MIN_NETWORK_PROTOCOL_VERSION is likewise TBD on both networks
%% (zip-0258.md:72-74).
From activation, three consensus rules seal the Orchard pool to
\emph{spend-only}, and they bind every transaction regardless of version:
\begin{itemize}[leftmargin=*]
\item a coinbase transaction must carry an empty Orchard component;
\item every Orchard-pool Action must have cross-address transfers disabled
($\mathsf{enableCrossAddress} = 0$, \S\ref{sec:iw-cross}) --- enforced by the
Action-circuit \emph{verifying key}, which consensus selects by branch, so a
v5 transaction cannot bypass the rule;
\item no new value may enter the pool:
$v_{\mathrm{balance}}^{\mathsf{Orchard}} \ge 0$ per transaction
(\S\ref{sec:iw-turnstile}).
\end{itemize}
All three are ZIP-258 rules; ZIP-229 states the second's
version-independence. Notably, \texttt{zcashd} is not expected to implement
NU6.3 at all; \texttt{zebra} is expected to be the only full-validator
implementation from activation onward (ZIP-258).

\paragraph{Keys are protocol-scoped.} Orchard spending-key and viewing-key
material grants authority to spend or view notes in \emph{both} pools; a
receiver, and the corresponding $\ivk$, is scoped to the Orchard protocol,
not to a pool (ZIP-229; ZIP-326). The entire key ladder of
\S\ref{sec:orchard-keys} therefore survives unchanged: an existing wallet's
keys and addresses work in Ironwood as they stand. What distinguishes the
pools is state, not identity: separate, independent note commitment trees
and nullifier sets (ZIP-229), separate anchors, and separate chain value
pool balances. The Ironwood tree starts empty at activation and uses the
identical depth-$32$ $\mathsf{MerkleCRH}^{\mathsf{Orchard}}$ construction of
\S\ref{sec:tree} (capacity $2^{32}$, ZIP-258); the Orchard tree keeps
growing after activation --- every Orchard-pool Action still appends a
$\cmx$, since spends under the cross-address restriction are paired with
fabricated or dummy outputs (\S\ref{sec:iw-cross}) --- and its anchors
remain live for spend proofs. Nullifier derivation itself
(\S\ref{sec:nullifier}) is unchanged in both pools; a note's nullifier is
checked only against its own pool's set.

\paragraph{The v6 transaction format.} A v6 transaction
($\texttt{V6\_TX\_VERSION} = 6$, version group
$\mathtt{0xD884B698}$; \texttt{librustzcash
zcash\_protocol/src/constants.rs}) is the v5 format --- with
$\mathsf{enableSpendsOrchard}/\mathsf{enableOutputsOrchard}$ renamed to
$\mathsf{enableSpends}/\mathsf{enableOutputs}$ and bit~2 of
$\mathsf{flagsOrchard}$ now carrying $\mathsf{enableCrossAddress}$ --- plus
an Ironwood component that reuses the \texttt{OrchardAction} encoding
byte-for-byte ($820$ bytes per Action; ZIP-229):
...
\midrule
\texttt{nActionsIronwood} & \texttt{compactSize} & Action count $n$ \\
\texttt{vActionsIronwood} & $820 \cdot n$ & \texttt{OrchardAction} encoding \\
\texttt{flagsIronwood} & $1$ & same bit layout as \texttt{flagsOrchard} \\
\texttt{valueBalanceIronwood} & $8$ &
net flow $v_{\mathrm{balance}}^{\mathsf{Ironwood}}$ \\
\texttt{anchorIronwood} & $32$ & a root of the \emph{Ironwood} tree \\
\texttt{proofsIronwood} & $2720 + 2272 \cdot n$ & aggregate Halo~2 proof \\
\texttt{vSpendAuthSigsIronwood} & $64 \cdot n$ & one SpendAuthSig per Action \\
\texttt{bindingSigIronwood} & $64$ & the pool's binding signature \\
\bottomrule
\end{tabular}
\caption{The Ironwood component of a v6 transaction (ZIP-229). The fields
after the count are present iff $n > 0$. The proof size formula is the same
$2720 + 2272 n$ as the Orchard component
(Table~\ref{tab:zcash-history}).}\label{tab:iw-v6}
\end{table}

Value conservation runs per pool: the Ironwood component has its own
balancing value $v_{\mathrm{balance}}^{\mathsf{Ironwood}}$ and its own
binding signature, constructed exactly as in \S\ref{sec:value} over the
Ironwood Actions' $\cvsym^{\mathsf{net}}$ values. ZIP-229 states the
note-level situation exactly: every Ironwood-pool output note uses the
quantum-recoverable note plaintext format (lead byte $\mathtt{0x03}$,
\S\ref{sec:iw-note}), no Orchard-pool output note does, and this is ``the
only note-level distinction between the two pools''.

\paragraph{The v6 digest.} The ZIP-244 tree of \S\ref{sec:value} gains an
Ironwood branch,
\[
\mathsf{sighash} := \mathrm{BLAKE2b\text{-}256}\bigl(\texttt{ZcashTxHash\_}
\,\|\, \mathsf{branchid},\ d_{\mathrm{hdr}} \,\|\, d_{\mathrm{trans}}
\,\|\, d_{\mathrm{Sap}} \,\|\, d_{\mathrm{Orch}} \,\|\,
d_{\mathrm{Irw}}\bigr),
\]
with new personalisations \texttt{ZTxIdIronwd\_H\_v6},
\texttt{ZTxIdIrnActCH\_v6}, \texttt{ZTxIdIrnActMH\_v6},
\texttt{ZTxIdIrnActNH\_v6}, \texttt{ZTxAuthIrnwdH\_v6} and revised v6
personalisations for the Sapling and Orchard branches
(\texttt{ZTxIdSSpendNH\_v6}, \texttt{ZTxAuthSapliH\_v6},
\texttt{ZTxIdOrchardH\_v6}, and \texttt{ZTxAuthOrchaH\_v6}); an absent
component
hashes the empty string under its personalisation (ZIP-229). This digest tree
is \emph{specified}, not deployed at \texttt{bef8a27}: there crate
\texttt{orchard}'s \texttt{bundle/commitments.rs} (\texttt{:7--11}) defines only
five non-versioned personalisations ---
\texttt{ZTxIdOrchardHash}, \texttt{ZTxIdOrcActCHash},
\texttt{ZTxIdOrcActMHash}, \texttt{ZTxIdOrcActNHash},
\texttt{ZTxAuthOrchaHash} --- and
\texttt{hash\_bundle\_txid\_data} applies no per-pool or per-version
personalisation dispatch --- its \texttt{format} argument feeds only the flags
byte (\texttt{:62}) --- so none of the \texttt{\_v6} strings exist in
\texttt{src/}. In \texttt{librustzcash} at \texttt{e30517e4} only the two
Sapling personalisations \texttt{ZTxIdSSpendNH\_v6} and
\texttt{ZTxAuthSapliH\_v6} are production in
\texttt{zcash\_primitives}, \texttt{transaction/txid.rs} (lines $44$
and $54$); the Orchard and Ironwood \texttt{\_v6} strings appear there only as
expected empty-hash literals in \texttt{transaction/tests.rs}. One structural
change
rides along: v6 moves the Sapling, Orchard, and Ironwood \emph{anchors} from
effecting data (under the txid) to authorizing data (under the auth digest,
committed only when the component spends). A transaction can then be
re-anchored --- its membership proofs rebased onto a later root --- without
changing its txid, a design ZIP-229 credits to Penumbra.

\paragraph{One circuit, one verifying key.} Both pools share the post-NU6.3
Action circuit and its keys. In crate \texttt{orchard} at \texttt{bef8a27} the
bundle's flag layout is chosen by the \texttt{BundleFormat} enum ---
\texttt{PreNu6\_3} or \texttt{Nu6\_3} (\texttt{src/bundle.rs:89--93}) --- and
the circuit by \texttt{OrchardCircuitVersion} --- \texttt{InsecurePreNu6\_2},
\texttt{FixedPostNu6\_2}, or \texttt{Ironwood}
(\texttt{src/circuit.rs:125--135}). For NU6.3 Actions the built circuit
version is \texttt{Ironwood} (\texttt{circuit\_version()},
\texttt{src/builder.rs:1251}) in both pools. The cross-address restriction is
gated by \texttt{supports\_cross\_address\_restriction()} on
\texttt{OrchardCircuitVersion} --- true only for \texttt{Ironwood}
(\texttt{src/circuit.rs:147}) --- and by \texttt{Flags::cross\_address\_enabled()}
(\texttt{src/bundle.rs:216}), which honours the flag bit only under
\texttt{BundleFormat::Nu6\_3} (\texttt{src/bundle.rs:283--284}). The pools are
distinguished by their trees, nullifier sets, balances, and component
position --- not by separate circuits.

\subsection{The Ironwood note: lead byte 0x03 and the hashed
trapdoor}\label{sec:iw-note}

An Ironwood note is the same tuple $\mathbf{n} = (\mathsf{addr}, v, \rho,
\psi, \rcm)$ as Definition~\ref{def:note}, committed by the same
\NoteCommit\ of \S\ref{sec:notes}; ZIP-2005 carries the pool distinction on
the note \emph{plaintext}, through its lead byte. What changes is that lead
byte and, through it, the derivation of $\rcm$ (ZIP-2005, Proposed;
activation bound to NU6.3). The format is \emph{specified} --- ZIP-2005
\S\,4.7.3, landed in \texttt{protocol.tex} at \texttt{zips} commit
\texttt{69610984} --- but undeployed in crate \texttt{orchard} at
\texttt{bef8a27}, whose \texttt{Note::from\_parts} (\texttt{src/note.rs:182})
still takes the four components $(\mathsf{recipient}, v, \rho,
\mathsf{rseed})$ with no version tag and whose \texttt{Note::rcm()} is the
legacy single-tag trapdoor of Remark~\ref{rem:iw-legacy-rcm}.
...
height $h$ --- one of $\mathsf{SaplingPool}$, $\mathsf{OrchardPool}$, or
$\mathsf{IronwoodPool}$. Then
\[
\mathsf{allowedLeadBytes}^{\mathsf{pool}}(h) :=
\begin{cases}
\{\mathtt{0x01}\}, & h < h_{\mathsf{Canopy}}, \\
\{\mathtt{0x01}, \mathtt{0x02}\}, & h_{\mathsf{Canopy}} \le h <
h_{\mathsf{Canopy}} + \mathsf{ZIP212GracePeriod}, \\
\{\mathtt{0x02}\}, & \text{afterwards, if } \mathsf{pool} \ne
\mathsf{IronwoodPool}, \\
\{\mathtt{0x03}\}, & \text{otherwise}.
\end{cases}
\]
For Ironwood-pool note plaintexts the only allowed lead byte is
$\mathtt{0x03}$ (ZIP-2005 \S\,3.2.1 change; landed in \texttt{protocol.tex}
at \texttt{zips} commit \texttt{69610984}). Ironwood coinbase outputs must
carry it as well, the ZIP-213 analogue of the Sapling/Orchard
$\mathtt{0x02}$ rule (ZIP-258).
\end{definition}

\begin{definition}[Ironwood note derivations]\label{def:iw-derive}
For a lead-byte-$\mathtt{0x03}$ note with seed $\mathsf{rseed}$ and
$\rho := \nfsym_{\mathrm{old}}$, the sender derives, \emph{in this order},
\begin{align*}
\mathsf{esk} &:= \mathrm{ToScalar}\bigl(\mathsf{PRFexpand}_{\mathsf{rseed}}
(\mathtt{0x04} \,\|\, \rho)\bigr) \in \F_{p_{\Vesta}}, \\
\psi &:= \mathrm{ToBase}\bigl(\mathsf{PRFexpand}_{\mathsf{rseed}}
(\mathtt{0x09} \,\|\, \rho)\bigr) \in \F_{p_{\Pallas}}, \\
\rcm &:= \mathrm{ToScalar}\bigl(\mathsf{PRFexpand}_{\mathsf{rseed}}
(\mathsf{pre\_rcm})\bigr) \in \F_{p_{\Vesta}}, \\
\mathsf{pre\_rcm} &:= \mathtt{0x0B} \,\|\, \gd^\star \,\|\, \pkd^\star
\,\|\, \mathrm{LE}_{64}(v) \,\|\, \mathrm{LE}_{256}(\rho) \,\|\,
\mathrm{LE}_{256}(\psi),
\end{align*}
each field in its canonical byte encoding: the points star-encoded ($32$
bytes each), $v$ as $8$ and $\rho, \psi$ as $32$ little-endian bytes, so
$\mathsf{pre\_rcm}$ is a $137$-byte string (ZIP-2005 \S\,4.7.3 change;
\texttt{protocol.tex} at \texttt{zips} commit \texttt{69610984}, where the
hash is named $\mathsf{H}^{\mathsf{rcm,Orchard}}_{\mathsf{rseed}}$).
\end{definition}

The $\mathsf{esk}$ and $\psi$ derivations are byte-identical to
\S\ref{sec:noteenc}; only $\rcm$ changes, and with it the order --- $\rcm$
now comes last because its input includes $\psi$ (ZIP-2005). The same
$\gd^\star, \pkd^\star$ encodings feed both $\mathsf{pre\_rcm}$ and the
\NoteCommit\ input of \S\ref{sec:notes}. The purpose is post-quantum
\emph{binding}: every note field now traverses BLAKE2b on the way to the
trapdoor, so an adversary constrained to treat $\mathsf{PRFexpand}$ as a
random oracle cannot vary any field without changing $\rcm$
(\S\ref{sec:pq-zip2005}, which owns the security analysis, the recovery
statement, and ZIP-2005's specified-but-undeployed
$\mathsf{qsk}/\mathsf{qk}$ key branch). Every Ironwood-pool output note is a
recoverable note; no Sapling or Orchard-pool output note is (ZIP-2005).

\begin{remark}[The legacy derivation, historically]\label{rem:iw-legacy-rcm}
Lead-byte-$\mathtt{0x02}$ notes --- all Orchard-pool notes, before and after
NU6.3 --- derive
$\rcm := \mathrm{ToScalar}(\mathsf{PRFexpand}_{\mathsf{rseed}}(\mathtt{0x05}
\,\|\, \rho))$ as in \S\ref{sec:noteenc}. The specification packages both
under one dispatcher,
$\mathsf{Derive\_rcm}^{\mathsf{Orchard}}_{\mathsf{rseed}}(\mathsf{leadByte},
\ldots)$, selecting $\mathtt{0x05} \,\|\, \rho$ for $\mathtt{0x02}$ and the
hashed $\mathsf{pre\_rcm}$ for $\mathtt{0x03}$ (ZIP-2005 \S\,4.7.3 change).
The deployed \texttt{Note::rcm()} at \texttt{bef8a27} implements only this
legacy branch: $\rcm := \mathrm{ToScalar}(\mathsf{PRFexpand}_{\mathsf{rseed}}
(\mathtt{0x05} \,\|\, \rho))$ (\texttt{src/note.rs:132--136}), with
\texttt{PrfExpand::ORCHARD\_RCM} $= \mathtt{0x05}$ (\texttt{zcash\_spec}~0.2.1,
\texttt{src/prf\_expand.rs:88}). It carries no version dispatch and no
$\mathtt{0x03}$/hashed-$\mathsf{pre\_rcm}$ branch; the ZIP-2005 derivation of
Definition~\ref{def:iw-derive} is specified but not yet implemented there.
\end{remark}

\paragraph{Decryption is reordered.} Trial decryption
(\S\ref{sec:noteenc}) rederives the note's quantities to check the
commitment, and the lead-byte-$\mathtt{0x03}$ $\rcm$ depends on $\gd, \pkd$,
and $\psi$; the specification therefore reorders both the $\ivk$ and $\fvk$
decryption procedures to recover $\gd$ and $\pkd$ first, then $\psi$, then
$\rcm$ (ZIP-2005 \S\S\,4.20.2--4.20.3 changes). No such routing is deployed
at \texttt{bef8a27}: crate \texttt{orchard} has a single \texttt{OrchardDomain}
(\texttt{src/note\_encryption.rs:84}) that hardcodes the lead byte, rejecting
any plaintext with $\mathsf{plaintext}[0] \ne \mathtt{0x02}$ (\texttt{:63})
and emitting $\mathtt{0x02}$ on encryption (\texttt{:172}), with no
$\mathtt{0x03}$ branch and no \texttt{IronwoodDomain}. Routing the two lead
bytes to two domains is the implementation ZIP-2005's reordered procedures
call for, still to be written. Because keys are protocol-scoped
(\S\ref{sec:iw-pools}), the \emph{same} $\ivk$ trial-decrypts both pools ---
one key, two domains (ZIP-326; \texttt{zcash\_client\_backend
src/scanning.rs}). The light-client surface follows suit: compact blocks
gain an \texttt{ironwoodActions} list (the same
\texttt{CompactOrchardAction} message) and an
\texttt{ironwoodCommitmentTreeSize}, and the synchronisation of
\S\ref{sec:noteenc} runs per pool against its own tree
(\texttt{zcash\_client\_backend proto/compact\_formats.proto}).

\subsection{The cross-address restriction in the Orchard
pool}\label{sec:iw-cross}

The rule that seals the Orchard pool most deeply is in-circuit: from NU6.3,
every Orchard-pool Action must have cross-address transfers disabled, so
that value inside the pool can no longer move between addresses --- an
Orchard-pool spend can pay only its own receiver. The motivation is the
recoverability guarantee of \S\ref{sec:pq-zip2005}: legacy $\mathtt{0x02}$
notes are unrecoverable, so consensus stops their circulation and funnels
value out of the pool, while still letting owners spend
(\S\ref{sec:iw-turnstile}).

\begin{definition}[Action statement, the A10 constraint in full]\label{def:iw-action}
The tenth public input of Definition~\ref{def:action},
$\mathsf{disableCrossAddress}$, sits at instance index $9$ (after
$\mathsf{anchor}$, the two $\cvsym^{\mathsf{net}}$ coordinates,
$\nfsym_{\mathrm{old}}$, the two $\rk$ coordinates, $\cmx$,
$\mathsf{enableSpends}$, $\mathsf{enableOutputs}$), and constraint A10 of
the relation is, writing $\delta := \mathsf{disableCrossAddress}$:

\medskip\noindent
\textbf{[A10] Cross-address gating} --- when $\delta \ne 0$, the new note's
receiver equals the spent note's:
\begin{align*}
...
0,
\end{align*}
four coordinate equalities forcing $\gd_{\mathrm{old}} = \gd_{\mathrm{new}}$
and $\pkd_{\mathrm{old}} = \pkd_{\mathrm{new}}$ as full curve points ---
equality of receivers, not merely of $x$-coordinates. The constraint shape
is the product gate A3 already uses ($v_{\mathrm{old}} \cdot (\mathsf{root}
- \mathsf{anchor}) = 0$), reused on four extra rows of the same
\texttt{q\_orchard} gate (crate \texttt{orchard} at \texttt{bef8a27}
\texttt{src/circuit.rs}: instance offsets \texttt{ANCHOR} through
\texttt{DISABLE\_CROSS\_ADDRESS}, rows assigned in
\texttt{synthesize\_cross\_address\_checks}; the historical nine-element
instance encoding is commitment-identical when $\delta = 0$, so pre-NU6.3
proofs and keys are untouched).
\end{definition}

\paragraph{Wire encoding and polarity.} The flag travels as bit~2 of the
component's flags byte, in the \emph{enabled} sense
($\mathsf{enableCrossAddress}$; bits are spends $\mathtt{0b001}$, outputs
$\mathtt{0b010}$, cross-address $\mathtt{0b100}$ --- \texttt{librustzcash
pczt/src/orchard.rs} carries both canonical values,
\texttt{0b0000\_0011} for Orchard and \texttt{0b0000\_0111} for
Ironwood). The polarity is backward-compatible by design: bit~$2 = 0$ ---
the restricted state consensus requires for post-NU6.3 Orchard-pool spends
--- is exactly what v5 signers treating the bit as reserved-zero already
produce, so existing hardware signers (ZIP-229 names the Keystone) keep
signing v5 Orchard unshielding transactions without a firmware update. The
Ironwood component carries the same flag and sets it: cross-address
transfers stay enabled there.

\paragraph{Spending under the restriction.} An Action spends one note and
creates one (\S\ref{sec:orchard-walkthrough}); if the created note must pay
the spent note's own receiver, how does an owner pay anyone? By exiting the
pool: the real value leaves through
$v_{\mathrm{balance}}^{\mathsf{Orchard}} > 0$ (\S\ref{sec:iw-turnstile}),
and the Action's output slot is filled with a \emph{fabricated} zero-valued
note to the spent note's own receiver. The wallet must fill that output's
$C_{\mathrm{enc}}$ with \emph{random} bytes: a real encryption would let any
$\ivk$ holder --- including a quantum adversary who recovered $\ivk$ from a
published address, the very adversary of \S\ref{sec:pq-hndl} --- decrypt it
and link the Action's nullifier to the address (ZIP-326). In a PCZT the
fabricated output carries its recipient, value, and $\mathsf{rseed}$
explicitly, so a signer can classify it as a tolerable dummy rather than an
opaque payment (ZIP-326; \texttt{librustzcash pczt/src/orchard.rs}). One
gap remains open by construction: paying \emph{into} an Orchard-pool
receiver post-NU6.3 requires a spend at that same receiver, hence the
spending key, so consensus cannot fully prevent self-sends; ZIP-326 wallet
rules close the gap by forbidding sends to external Orchard-pool receivers.

\paragraph{Status of ZIP-2006.} The restriction is deployed and enforced by
consensus, but its owning ZIP does not yet exist as text: \texttt{zip-2006}
is a nine-line Reserved stub (``Restricting Transfers into the Orchard
Pool''; owner Daira-Emma Hopwood; created 2025-06-20; discussion
\texttt{zcash/zips\#1305}). \emph{Specified} (in ZIP-229/258): the flag
encoding, its consensus rule, and the verifying-key selection by branch.
\emph{Designed but unspecified}: the normative semantics --- ZIP-326 phrases
the restriction as equality of ``expanded receivers'' and carries a
\texttt{[TODO define]} in its own text; the
$\mathsf{enableCrossAddress} \to \mathsf{disableCrossAddress}$ instance
mapping and the dummy-spend treatment are likewise reconstructable only from
ZIP-229/258/326 and crate \texttt{orchard} at \texttt{bef8a27}, as Definition
\ref{def:iw-action} does.
%% TODO: when ZIP 2006 is published, replace the reconstruction above with
%% its normative text (expanded-receiver definition, instance mapping,
%% dummy-spend rules) and re-cite A10 against it.

\subsection{Migration and the turnstile}\label{sec:iw-turnstile}

Value leaves the Orchard pool through a \emph{turnstile}: a one-way,
publicly metered gate, the same instrument Zcash used for the Sprout
retirement. The consensus content is two rules already stated. Per
transaction, from activation,
\[
v_{\mathrm{balance}}^{\mathsf{Orchard}} \;\ge\; 0
\qquad \text{(any transaction version; ZIP-258)},
\]
so value may exit but never enter; and the chain maintains an Ironwood
analogue of every pool-balance rule (the ZIP-209 extension):
\[
\mathsf{IronwoodPoolBalance} \;:=\; -\sum_{\mathrm{tx}}
v_{\mathrm{balance}}^{\mathsf{Ironwood}}(\mathrm{tx}),
\]
zero before NU6.3, never negative, with the sum of all pool balances
bounded by $\mathsf{MAX\_MONEY}$ (ZIP-258). A migrating transaction spends
Orchard-pool notes with $v_{\mathrm{balance}}^{\mathsf{Orchard}} > 0$ ---
releasing value to the transaction's transparent value pool --- and absorbs
it into an Ironwood component with
$v_{\mathrm{balance}}^{\mathsf{Ironwood}} < 0$. Both fields are public: the
turnstile reveals the amounts crossing between pools in each transaction,
exactly as ZIP-229's privacy analysis states, and no per-transaction or
aggregate cap is specified anywhere. The remaining Ironwood consensus rules
are the Orchard ones of this volume, re-instantiated:
$\mathsf{anchorIronwood}$ must be an earlier block's final Ironwood
treestate (\S\ref{sec:tree}), the tree capacity is $2^{32}$, and the ZIP-221
history tree (\emph{FlyClient Guide}, \S``NU5: the commitment becomes one
of two'') gains three Ironwood fields (earliest root, latest root,
transaction count) aggregated exactly like the Orchard trio (ZIP-258).

\paragraph{Wallet posture.} Wallets SHOULD proactively move \emph{all}
funds --- transparent, Sprout, and Sapling included --- into the Ironwood
pool, the only pool whose output notes are recoverable, and must treat the
sweep as ongoing (ZIP-2005; \S\ref{sec:pq-zip2005}). The deployed change-routing
logic implements the turnstile's grain: change from Ironwood value stays in
Ironwood, and after activation change may return to the Orchard pool only
when the transaction spends Orchard notes \emph{and} strictly less value
returns than the spends remove; otherwise change is diverted to Ironwood
(function \texttt{select\_change\_pool}, \texttt{librustzcash
zcash\_client\_backend/src/fees/common.rs}). Scanning narrows
correspondingly: ZIP-326's scan ranges close the Orchard pool off ---
external receivers scan from the birthday only to activation, internal ones
until the wallet observes a zero Orchard balance after activation, a
watermark that is sound precisely because
$v_{\mathrm{balance}}^{\mathsf{Orchard}} \ge 0$ means a zero balance can
never become nonzero --- while Ironwood scans from the later of birthday
and activation to the tip (ZIP-326).

\paragraph{Status of ZIP-318.} The migration ZIP itself --- ``Orchard to
Ironwood Migration'' --- is a twelve-line Reserved stub (owners Schell Carl
Scivally and Pacu Gindre; created 2026-06-16; discussion
\texttt{zcash/zips\#1315}). Everything consensus-level above is specified
in ZIP-229/258; what ZIP-318 is expected to own --- batching, scheduling,
and amount-disclosure guidance in the style of the Sprout-to-Sapling
migration (ZIP-308), mitigating the public crossing amounts --- exists
nowhere as text and is an \emph{open problem}.
%% TODO: when ZIP 318 is published, absorb its wallet mechanics here and
%% reclassify.
%% TODO: ZIP 317 fee accounting across two Orchard-protocol bundles is
%% described as "updated" by ZIP 258; confirm the ZIP's logical-action
%% counting matches librustzcash's separate per-bundle padding
%% (zcash_client_backend fees/common.rs) before citing either.
\section{Post-quantum security of the Orchard protocol}\label{sec:pq-orchard}
...
acceptance event and mitigates the break completely. The Orchard protocol
--- both its pools --- has
...
commit \texttt{69610984} (2026-07-09); crate \texttt{orchard} at git commit
\texttt{bef8a27e} (v0.14.0, 2026-06-16) for circuit claims; and crate
\texttt{orchard} at \texttt{bef8a27} for the $\mathtt{0x03}$ note path; we
verified each cited artifact at these pins.
...
\texttt{69610984} contains no occurrence of ML-KEM, Kyber, ML-DSA, or a
...
\subsection{ZIP-2005: quantum recoverability of Ironwood
notes}\label{sec:pq-zip2005}
...
\texttt{69610984}); the scheduling follows the NU6.2 emergency circuit
correction (ZIP-257, Final), which is why the originally planned earlier
deployment slipped. The pool split itself is the subject of \S\ref{sec:iw};
what this subsection relies on is the seal. From activation the legacy
Orchard pool is spend-only: a coinbase transaction must carry an empty
Orchard component, every Orchard-pool Action must have
$\mathsf{enableCrossAddress} = 0$, and no new value may enter the pool
($v_{\mathrm{balance}}^{\mathsf{Orchard}} \ge 0$) --- the second rule
enforced by the branch-selected Action-circuit verifying key, the other two
as ordinary consensus rules (ZIP-258; ZIP-229).

\paragraph{Mechanism.} Ironwood note plaintexts carry lead byte
$\mathtt{0x03}$, and with it the hashed trapdoor derivation of
\S\ref{sec:noteenc}: $\rcm$ derives from the $\mathtt{0x0B}$-separated
$\mathsf{pre\_rcm}$ concatenating every note field
(Definition~\ref{def:iw-derive}), in place of the legacy
$\mathtt{0x05} \,\|\, \rho$ input. Every note field now traverses
...
ZIP-258 (Draft), all at \texttt{zips} commit \texttt{69610984} --- and are
