...
and from NU6.3 the $\mathsf{enableSpends}$ bit of $\mathsf{flagsIronwood}$
MUST be $0$ --- ZIP-229).
...
crate \texttt{orchard} 0.15.0 \texttt{src/circuit.rs}). The legacy NU5/NU6
...
\texttt{PostNu6\_3} in crate \texttt{orchard} 0.15.0
...
implementation and the specification --- here crate \texttt{orchard} 0.15.0
...
crate \texttt{orchard} 0.15.0, \texttt{bundle/commitments.rs}, and in
...
Action circuit and its keys: in crate \texttt{orchard} 0.15.0, the bundle
version is the pair of a \texttt{ValuePool} $\{\mathrm{Orchard},
\mathrm{Ironwood}\}$ and a \texttt{ProtocolVersion}
$\{\mathrm{InsecureV1}, \mathrm{V2}, \mathrm{V3}\}$;
\texttt{circuit\_version()} maps V3 to \texttt{PostNu6\_3} for both pools,
\texttt{note\_version()} maps Orchard to V2 and Ironwood to V3, and
\texttt{permits\_cross\_address\_transfers()} is false exactly for the pair
(V3, Orchard) (\texttt{src/lib.rs}, \texttt{src/bundle.rs}). The pools are
...
\NoteCommit\ of \S\ref{sec:notes}; the deployed implementation records the
distinction as a version tag on the note (\texttt{NoteVersion} $\{\mathrm{V2},
\mathrm{V3}\}$, with \texttt{Note::from\_parts} now taking one; crate
\texttt{orchard} 0.15.0 \texttt{src/note.rs}). What changes is the note
\emph{plaintext} lead byte and, through it, the derivation of $\rcm$
(ZIP-2005, Proposed; activation bound to NU6.3).
...
The deployed dispatch is \texttt{Note::rcm()} matching \texttt{NoteVersion}:
V2 to \texttt{rcm\_v2}, V3 to \texttt{rcm\_v3}, the latter one BLAKE2b-512
call personalised \texttt{Zcash\_ExpandSeed} over
\[
\mathsf{rseed} \,\|\, \mathtt{0x0B} \,\|\, \gd^\star \,\|\, \pkd^\star
\,\|\, \mathrm{LE}_{64}(v) \,\|\, \mathrm{LE}_{256}(\rho) \,\|\,
\mathrm{LE}_{256}(\psi)
\]
(crate \texttt{orchard} 0.15.0 \texttt{src/note.rs}, \texttt{rcm\_v3},
domain separator constant \texttt{0x0B}).
\end{remark}

\paragraph{Decryption is reordered.} Trial decryption
(\S\ref{sec:noteenc}) rederives the note's quantities to check the
commitment, and the V3 $\rcm$ depends on $\gd, \pkd$, and $\psi$; the
specification therefore reorders both the $\ivk$ and $\fvk$ decryption
procedures to recover $\gd$ and $\pkd$ first, then $\psi$, then $\rcm$
(ZIP-2005 \S\S\,4.20.2--4.20.3 changes). At the code level the routing is by
\emph{domain}: sealed marker types give \texttt{OrchardDomain} (expects lead
byte $\mathtt{0x02}$) and \texttt{IronwoodDomain} (expects $\mathtt{0x03}$),
each rejecting a plaintext whose lead byte differs (crate \texttt{orchard}
0.15.0 \texttt{src/note\_encryption.rs}). Because keys are protocol-scoped
...
\texttt{q\_orchard} gate (crate \texttt{orchard} 0.15.0
...
ZIP-229/258/326 and crate \texttt{orchard} 0.15.0, as Definition
...
\texttt{orchard} 0.15.0 (crates.io) for the $\mathtt{0x03}$ note path; we
